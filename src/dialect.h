...
namespace Liquid {
    ...
    // As such, you can either enable/disable features one at a time, or call `implementStrict`, `implementShopify`, or `implementPermissive` for the
    // defaults that are least/least/most permissive.
    struct Dialect {};
    ...
    struct StandardDialect :Dialect {
        ...
        static void implementShopify(Context& context) {
            implementStrict(context);
        }
