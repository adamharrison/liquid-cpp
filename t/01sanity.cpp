...
TEST(sanity, subtraction) {
    ...
    str = getInterpreter().renderTemplate(getCompiler().compile(ast), variable);
...
TEST(sanity, casestatement) {
    ...
    str = getInterpreter().renderTemplate(getCompiler().compile(ast), hash);
    ASSERT_EQ(str, "7");
}
...
TEST(sanity, assignments) {
    ...
    str = getInterpreter().renderTemplate(getCompiler().compile(ast), copyVariable);
    ASSERT_EQ(str, " 1");
